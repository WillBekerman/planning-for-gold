...
run_sim <- function(num_subjects=5000, num_outcomes=250, true_theta=1,
                    planning_sample_prop=0.2, alpha=0.05,
                    uc_dgp=TRUE, gamma=1, nsim=1000, nboot=250,
                    num_effects=5,num_covariates=20){
  ...
  for (sim in 1:nsim){
    ...
    if (uc_dgp){
      ...
      desired_trt_ratio_rv <- rbeta(n=1,shape1=10,shape2=10)
...
for (gamma_idx in 1:length(gammas)){
  ...
  saveRDS(data, file = paste0("NUC-df-","gamma",gamma,".rds"))
...
for (gamma_idx in 1:length(gammas)){
  ...
  saveRDS(data, file = paste0("UC-df-","gamma",gamma,".rds"))
